...
Let us now calculate the $OLR_{\nu_c}$ in the weak line limit. We also assume that the concentration of the gas is well-mixed: that $q(p)=q=const$. We again need to find the differential $d\bar{\mathcal{T}_{\nu_c}}(p',p_s)$ with respect to $p'$:
\begin{align*}
    d\bar{\mathcal{T}_{\nu_c}}(p',p_s)&=d\left( 
     ...
     \right)\\
     &= - d \left( 
        S\frac{q}{g\cos\tilde{\theta}}(p_s-p')
     \right)\\
     &=\frac{Sq}{g\cos\tilde{\theta}}\,dp'
\end{align*}
...
\begin{align*}
    ...
    +
    \int_{p_s}^{0}\pi B(\nu_c,T(p'))\,d\bar{\mathcal{T}_{\nu_c}}(p',p_s)\\
    &=F_+(\nu_c,0)(\Delta-S\mu)+\int_{p_s}^{0}\pi B(\nu_c,T(p'))\frac{Sq}{g\cos\tilde{\theta}}\,dp'
\end{align*}
...
I note here that, sadly, none of what we learnt will be applicable to calculating the radiative forcing of CO$_2$. That is becuase CO$_2$ has strongly \textbf{overlapping} lines, and so we cannot simply integrate over a single spectral line and sum up each contribution, as that would result in double counting.
