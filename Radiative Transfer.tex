...
If temperature \textit{decreases} with height, and therefore \textit{increases} with pressure, $T(p_{fin})<T(p_{in})$, then the infrared emitters at $p_{fin}$ will have a lower temperature and will then, following general properties of \hyperref[Blackbody Frequency]{Blackbody Radiation}, emit less radiation. As such, $OLR$ decreases. Conversely, if temperature \textit{increases} with height, and therefore \textit{decreases} with pressure, $T(p_{fin})>T(p_{in})$, then the infrared emitters at $p_{fin}$ will have a higher temperature and will emit more radiation, increasing $OLR$.
