...
\begin{align*}
  \frac{DU}{Dt} + \frac{p}{\rho}\vec{\nabla}\cdot\vec{u} = Q + Q_c
\end{align*}
where \(Q_c=\) the energy change due to a change in composition. For an ideal gas, \(U = c_v T\), and so has no dependence on composition (because there are no interactions between constituents of the gas), so \(Q_c = 0\) for an ideal gas. Some brief can show that for an ideal gas the thermoydnamic equation is given by Equation \ref{Energy Equation}:
\begin{align*}
  c_p \frac{DT}{Dt} - \frac{\beta T}{\rho} \frac{Dp}{Dt} = Q
\end{align*}
