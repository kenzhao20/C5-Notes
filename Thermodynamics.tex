...
In reality, the heat capacity \textit{per mole} (and \textit{not} per mass) at constant volume is $c_v^*\approx$ \qty{20.83}{\joule\per\kelvin\per\mole}. $R^*\approx$ \qty{8.314}{\joule\per\kelvin\per\mole}, so our prediction would be $c_v^*\approx$ \qty{20.79}{\joule\per\kelvin\per\mole}, which is actually very close to the measured value!
