...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[domain=4:0,dotted,thick,mymagenta] plot (\x,{1}) node[left] {$\frac{F\tau_s}{C_s}$};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[->] (0,-0.2) -- (0,4.2) node[above] {$T$};
        \draw[dashed,thick,myorange] plot (\x,{0}) node[below] {$T_d(t)=0$};
        \draw[domain=1:4,dotted,thick,mymagenta] plot (\x,{\x-1}) node[right] {$\frac{F\tau_s}{C_s}$};
        \draw[color=mydarkblue,ultra thick] plot (\x,{\x-1+exp(-\x)}) node[above] {$T_s(t) = \frac{G\tau_s^2}{C_s}\left( \frac{t}{\tau_s}-1+e^{-\frac{t}{\tau_s}} \right)$};
        \node[] at (1,-0.3) {$\tau_s$};
    \end{tikzpicture}
