...
Let us refer again to our solution written in Equation \ref{eq Dynamical Solution}. We can make a few simplifications when applying this to our model. First, \(\vec{f}=(F_{ext}/C_s,0)^T\), so we can set \([\vec{f}]_2=0\). 
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[dashed,ultra thick,myorange] plot (\x,{0}) node[below] {\(T_d(t)=0\)};
        \draw[domain=4:0,dotted,thick,mymagenta] plot (\x,{1}) node[left] {\(\frac{F}{\lambda+\lambda'+\gamma}\)};
        \draw[color=mydarkblue,ultra thick] plot (\x,{1-exp(-\x)}) node[above] {\(T_s(t) = \frac{F}{\lambda+\lambda'+\gamma}\left( 1-e^{-\frac{t}{\tau_s}} \right)\)};
        \node[] at (1,-0.3) {\(\tau_s\)};
    \end{tikzpicture}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\linewidth}
        \begin{align*}
            ...
            \\
            \therefore
            T_s(t)&\approx\frac{F}{\lambda}\left( 1 - \frac{\gamma+\lambda'}{\lambda+\gamma+\lambda'}e^{-\frac{t}{\tau_d}}\right)
        \end{align*}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[->] (-0.2,0) -- (4.2,0) node[right] {\(t\)};
        \draw[->] (0,-0.2) -- (0,4.2) node[above] {\(T\)};
        \draw[dashed,ultra thick,myorange] plot (\x,{2-2*exp(-\x)}) node[below] {\(T_d(t)=\frac{F}{\lambda}\left( 1-e^{-\frac{t}{\tau_d}} \right)\)};
        \draw[domain=4:0,dotted,thick,mymagenta] plot (\x,{2}) node[left] {\(\frac{F}{\lambda}\)};
        \draw[color=mydarkblue,ultra thick] plot (\x,{2-2*exp(-\x)/2}) node[above] {\(T_s(t) = \frac{F}{\lambda}\left( 1-\frac{\gamma+\lambda'}{\lambda+\gamma+\lambda'}e^{-\frac{t}{\tau_s}} \right)\)};
        \node[] at (1,-0.3) {\(\tau_d\)};
    \end{tikzpicture}
    \end{subfigure}
...
Notice how the response of \(T_d\) on this long timescale is pretty much exactly analogous to the response of \(T_s\) on a short timescale. Note that we found how \(T_s\) evolves by substituting our expression for \(T_d\) into \(T_s\).

We find then that on an \(\mathcal{O}(\tau_d)\) timescale, \(T_d\) equilibrates with a timescale of \(\tau_d\) to its new equilibrium value \(\frac{F}{\lambda}\). Note that this equilibrium value is completely ignorant to \(\lambda'\) and \(\gamma\), which represent out-of-equilibrium effects – as expected. This is why we call \(\lambda\) the \textbf{Equilibrium Climate Sensitivity Parameter} – it is what the climate tends towards in equilibrium. Meanwhile, \(T_s\) also equilibrates towards this \(\frac{F}{\lambda}\) value as well, but note that it already has a `headstart' at time \(t=0\) to get to this equilibrium value. It gets to this point via the short timescale response, where \(T_d\) effectively didn't respond fast enough to noticably change.

Here is the `stitched together' response of the system.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[domain=0:14]
        \draw[very thin,color=gray] (-0.1,-0.1) grid (13.9,3.9);
        \draw[->] (-0.2,0) -- (14.2,0) node[right] {\(t\)};
        \draw[->] (0,-0.2) -- (0,4.2) node[above] {\(T\)};
        \draw[domain=0:1,dashed,ultra thick,myorange] plot (\x,{0}) node[below] {\(T_d(t)=0\)};
        \draw[domain=0:1,color=mydarkblue,ultra thick] plot (\x,{1-exp(-\x*10)}) node[above] {\(T_s(t) = \frac{F}{\lambda+\lambda'+\gamma}\left( 1-e^{-\frac{t}{\tau_s}} \right)\)};
        \node[] at (0.1,-0.3) {\(\tau_s\)};
        \draw[domain=14:0,dotted,thick,mymagenta] plot (\x,{1}) node[left] {\(\frac{F}{\lambda+\lambda'+\gamma}\)};
        \draw[domain=1:14,dashed,ultra thick,myorange] plot (\x,{2-2*exp(-\x/10)}) node[below] {\(T_d(t)=\frac{F}{\lambda}\left( 1-e^{-\frac{t}{\tau_d}} \right)\)};
        \draw[domain=14:0,dotted,thick,mymagenta] plot (\x,{2}) node[left] {\(\frac{F}{\lambda}\)};
        \draw[domain=1:14,color=mydarkblue,ultra thick] plot (\x,{2-2*exp(-\x/10)/2}) node[above] {\(T_s(t) = \frac{F}{\lambda}\left( 1-\frac{\gamma+\lambda'}{\lambda+\gamma+\lambda'}e^{-\frac{t}{\tau_s}} \right)\)};
        \node[] at (5,-0.3) {\(\tau_d\)};
        \draw [
            thick,
            decoration={
        ...
        node [pos=0.5,anchor=north,yshift=-0.7cm] {short};
        \draw [
            thick,
            decoration={
                brace,
        ...
        node [pos=0.5,anchor=north,yshift=-0.7cm] {long}; 
    \end{tikzpicture}
    \caption{The short and long timescale responses (stitched together) of the system subject to constant forcing. The response of \(T_s\) and \(T_d\) are drawn as a \textcolor{mydarkblue}{filled blue line \rule{0.25cm}{0.25cm}} and \textcolor{myorange}{dashed orange line \rule{0.25cm}{0.25cm}}, respectively. The \textcolor{mymagenta}{dotted magenta lines \rule{0.25cm}{0.25cm}} indicate what both \(T_s\) and \(T_d\) tend toward on a short and long timescale.}
    \label{const F figure}
\end{figure}
...
So far we have only written the solution in terms of \(F_{ext}(t)\), the external radiative forcing. However, we do not directly control radiative forcing – we only control CO\(_2\) emissions! To represent this, let us first consider Equation \ref{eq Dynamical Solution} and write it in terms of \(\frac{dg_i}{dt}\) by integrating by parts.

[UNDER CONSTRUCTION]

\section{Analogous Systems: The Carbon Cycle}

We can apply the previous method to similar systems. Consider, for example, a carbon budget equation, given by:
\begin{align}
...
\hline
\hline
\(E(t)\) & The perturbation in CO\(_2\) concentrations due to emissions from human or natural.\\
\hline
\(-r(C_a-C_b)\) & The CO\(_2\) flux from the atmosphere into the biosphere/surface-ocean. \\ 
\hline
\(-s(C_b-C_d)\) & The CO\(_2\) flux from the biosphere/surface-ocean into the deep ocean. Similar to temperature, this term is typically small.\\
\hline
\end{tabular}
\end{figure}

We can use the exact same methods as before to solve/understand this system. The first (official but more long-winded) method is to write this in terms of a matrix equation and diagonalise the matrix:
\begin{align}
    \left( \begin{array}{c}
        \dot{C}_a \\\\ 
        \dot{C}_b \\\\
    ...
    \left( \begin{array}{ccc}
        -\frac{r}{R_a} & \frac{r}{R_a} & 0
        \\\\
        \frac{r}{R_b} & -\frac{r+s}{R_b} & \frac{s}{R_b}
        \\\\
        0 & \frac{s}{R_d} & -\frac{s}{R_d}
    \end{array}
    \right)
    \left(  \begin{array}{c}
        {C}_a \\\\ 
    ...
    \right)
\end{align}
However, I'll mainly focus on the second method here. I'll also plot the responses to a constant injection of CO\(_2\) into the atmosphere (i.e., we consider the case where \(E(t)=E_0=const\)).

\subsubsection{The Short Timescale Response}

The short timescale response is found by setting \(C_b\approx C_d\approx 0\), because both \(C_b\) and \(C_d\) respond very slowly on this timescale (since \(R_d, R_b\gg R_a\)). The response of the system is set by the timescale \(t_a\sim\mathcal{O}\left(\frac{R_a}{r}\right)\), and \(C_a(t)\), \(C_b(t)\), and \(C_d(t)\) evolve according to the following equations:

\begin{figure}[H]
\centering
\begin{subfigure}{0.48\linewidth}
    \begin{align*}
        &R_a \dot{C_a} = E(t)-r C_a
        \\
        &C_b = 0
        \\
        &C_d = 0
    ...
    For \(E(t)=E_0\):
    \begin{gather*}
        C_a(t) = \frac{E_0}{r}\left( 1-e^{-\frac{t}{\tau_a}} \right)
        \\
        \tau_a = \frac{R_a}{r}
    \end{gather*}
\end{subfigure}
...
\begin{subfigure}{0.48\linewidth}
    ...
    \begin{tikzpicture}[domain=0:4]
        \draw[very thin,color=gray] (-0.1,-0.1) grid (3.9,3.9);
        \draw[->] (-0.2,0) -- (4.2,0) node[right] {\(t\)};
        \draw[->] (0,-0.2) -- (0,4.2) node[above] {\(C\)};
        \draw[dashed,ultra thick,myorange] plot (\x,{0}) node[above] {\(E_b(t)=0\)};
        \draw[dash dot,ultra thick,mydarkblue] plot (\x,{0}) node[below] {\(E_d(t)=0\)};
        \draw[domain=4:0,dotted,thick,mymagenta] plot (\x,{1}) node[left] {\(\frac{E_0}{r}\)};
        \draw[ultra thick] plot (\x,{1-exp(-\x)}) node[above] {\(E_a(t) = \frac{E_0}{r}\left( 1-e^{-\frac{t}{\tau_a}} \right)\)};
        \node[] at (1,-0.3) {\(\tau_a\)};
    \end{tikzpicture}
\end{subfigure}
    \caption{The short timescale response of the system subject to constant forcing. The response of \(C_a\), \(C_b\), and \(C_d\) are drawn as a \textcolor{black}{filled black line \rule{0.25cm}{0.25cm}}, \textcolor{myorange}{dashed orange line \rule{0.25cm}{0.25cm}}, and \textcolor{mydarkblue}{dash-dotted blue line \rule{0.25cm}{0.25cm}}, respectively. The \textcolor{mymagenta}{dotted magenta line \rule{0.25cm}{0.25cm}} indicates what \(C_a\) tends towards on a short timescale.}
    \label{fig Short E}
\end{figure}

\subsubsection{The Medium Timescale Response}

The medium timescale response is found by setting \(\dot{C_b} \approx\dot{C_a} \approx 0\), because both  We make the first approximation because \(C_d\) responds very slowly on this timescale (since \(R_d\gg R_b\)), and we make the second approximation because \(C_a\) evolves rapidly into equilibrium with \(C_b\) on this timescale (since \(R_a\ll R_b\)), and proceeds to evolve quasi-steadily. The response of the system is set by the timescale \(t_b\sim\mathcal{O}\left(\frac{R_b}{s}\right)\), and \(C_a(t)\), \(C_b(t)\), and \(C_d(t)\) evolve according to the following equations:

\begin{figure}[H]
\centering
\begin{subfigure}{0.48\linewidth}
    \begin{align*}
        &C_a(t)= C_b(t)+\frac{E(t)}{r}
        \\
        &R_b \dot{C_b}=E(t)-sC_b
        \\
        &C_d = 0
    \end{align*}
    ...
    For \(E(t)=E_0\):
    \begin{gather*}
        C_b(t) = \frac{E_0}{s}\left( 1-e^{-\frac{t}{\tau_b}} \right)
        \\
        \tau_b = \frac{R_b}{s}
    \end{gather*}
\end{subfigure}
...
\begin{subfigure}{0.48\linewidth}
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[dashed,ultra thick,myorange] plot (\x,{1.5-1.5*exp(-\x)}) node[below] {\(C_b(t) = \frac{E_0}{s}\left( 1-e^{-\frac{t}{\tau_b}} \right)\)};
        \draw[domain=4:0,dotted,thick,mymagenta] plot (\x,{1.5}) node[left] {\(\frac{E_0}{s}\)};
        \draw[mydarkblue,ultra thick,dash dot] plot (\x,{0}) node[below] {\(C_d(t)=0\)};
        \draw[ultra thick] plot (\x,{2.5-1.5*exp(-\x)}) node[above] {\(C_a(t)=\frac{E_0}{r}+\frac{E_0}{s}\left( 1-e^{-\frac{t}{\tau_b}} \right)\)};
        \node[] at (1,-0.3) {\(\tau_b\)};
    \end{tikzpicture}
\end{subfigure}
    \caption{The medium timescale response of the system subject to constant forcing. The response of \(C_a\), \(C_b\), and \(C_d\) are drawn as a \textcolor{black}{filled black line \rule{0.25cm}{0.25cm}}, \textcolor{myorange}{dashed orange line \rule{0.25cm}{0.25cm}}, and \textcolor{mydarkblue}{dash-dotted blue line \rule{0.25cm}{0.25cm}}, respectively. The \textcolor{mymagenta}{dotted magenta line \rule{0.25cm}{0.25cm}} indicates what \(C_b\) tends towards on a short timescale.}
    \label{fig Medium E}
\end{figure}

\subsubsection{The Long Timescale Response}

The medium timescale response is found by setting \(C_b\approx 0\) and \(\dot{C_a} \approx 0\) because foth \(C_d\) and \(C_a\) evolve rapidly into equilibrium with \(C_d\) on this timescale (since \(R_a,R_b\ll R_d\)), and proceed to evolve quasi-steadily. \(C_a(t)\), \(C_b(t)\), and \(C_d(t)\) evolve according to the following equations:
...
\begin{subfigure}{0.48\linewidth}
    ...
    \begin{tikzpicture}[domain=0:4]
        ...
        \draw[->] (-0.2,0) -- (4.2,0) node[right] {\(t\)};
        \draw[->] (0,-0.2) -- (0,4.2) node[above] {\(C\)};
        \draw[dashed,ultra thick,myorange] plot (\x,{0.5*\x+1}) node[below] {\(C_b(t)=E_0\left( \frac{t}{R_d}+\frac{1}{s} \right)\)};
        \draw[ultra thick] plot (\x,{0.5*\x+2.5}) node[left] {\(C_b(t)=E_0\left( \frac{t}{R_d}+\frac{1}{s} +\frac{1}{r}\right)\)};
        \draw[dash dot, mydarkblue,ultra thick] plot (\x,{0.5*\x}) node[below] {\(C_d(t)=\frac{E_0}{R_d}t\)};
    \end{tikzpicture}
\end{subfigure}
    \caption{The long timescale response of the system subject to constant forcing. The response of \(C_a\), \(C_b\), and \(C_d\) are drawn as a \textcolor{black}{filled black line \rule{0.25cm}{0.25cm}}, \textcolor{myorange}{dashed orange line \rule{0.25cm}{0.25cm}}, and \textcolor{mydarkblue}{dash-dotted blue line \rule{0.25cm}{0.25cm}}, respectively. There is no \textcolor{mymagenta}{dotted magenta line \rule{0.25cm}{0.25cm}}, because a steady-state is impossible in this system.}
    \label{fig Long E}
\end{figure}

It is difficult to define a timescale here using the second method – we cannot simply compare \(C_d\) to \(\dot{C_d}\). 

If we defer to diagonalising the matrix, we find that the corresponding eigenvalue for this normal mode is \(0\), and so the timescale is actually \(t_d\sim\infty\)! This reflects the fact that our carbon budget system is closed (unlike the temperature model), and once you pump in carbon (represented by the \(E(t)\) term), it has nowhere to go other than to stay in the system. As such, if \(E(t)>0\) for all \(t\), the system will never relax to a steady state, and \(Ca\), \(C_b\), and \(C_d\) will all grow without bound (as seen clearly in Figure \ref{fig Long E}).

This system, again, may be solved by finding eigenvalues and eigenvectors, but I would recommend doing the second quasi-steady method, and solving the single ODE governing \(C_d\) (assuming that \(C_a\) and \(C_b\) are quasisteady).
...
Suppose we take \(n\) measurements (or make \(n\) observations), which we encode in some matrix \(\vec{y}\in\mathbb{R}^m\), with some random error \(\vec{\epsilon}\in\mathbb{R}^m\), in order to estimate some values \(\vec{x}\in\mathbb{R}^n\) where \(n<m\). The measurements \(\vec{y}\) are some (known) function of \(\vec{x}\) like so: \(\vec{y}=f(\vec{x})+\vec{\epsilon}\) where \(f:\mathbb{R}^n\to\mathbb{R}^m\).

In some cases, \(f\) is linear, and we can represent \(f(\vec{x})=\hat{H}\vec{x}\) where \(\hat{H}\in\mathbb{R}^m\times\mathbb{R}^n\) (i.e., \(\hat{H}\) is an \(m\times n\) matrix). We know \(\vec{y}\), but we wish to estimate \(\vec{x}\). Since \(m<n\), the system is overdetermined: we cannot simply invert \(\hat{H}\) and solve for \(\vec{x}\), and this is exacerbated by (unknown) the error \(\vec{\epsilon}\).
...
\begin{fact}{Estimator Recipe}{estimator box}\label{estimator box}
    ...
    \begin{enumerate}
        ...
        \item Yeet the pseudo-inverse \(\hat{K}\) onto the observations \(\vec{y}\) (which you should be given), i.e., estimate \(\vec{x}\) as \(\vec{\hat{x}}=\hat{K}\vec{y}_0\).
        \item Watch the examiners award you The Scott Prize (£750) for the Best Performance in the MPhys examination.
        \item Donate those £750 (and maybe the prize as well) to the author(s) of these lecture notes.
    \end{enumerate}
\end{fact}
