...
This will be a bit convoluted, but we will introduce three coordinate systems, all of which rotate with the Earth\footnote{We've already dealt with the fact that the coordinate system is rotating in the previous section!}. In this section (\hyperref[s Spherical Coordinates]{Space-Dependent Coordinate Systems Pt. 1}) we will introduce the first two. We will only introduce the third coordinate system in \hyperref[s Local Cartesian Coordinates]{Space-Dependent Coordinate Systems Pt. 2}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture},
        ...
        \begin{scope}[3d view={110}{25.26}]
            ...
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0,0,0.5) node[anchor=west] {\(\hat{z}'\)};
        \end{scope}
        \begin{scope}[3d view={140}{25.26}]
            % Circle in x-y plane
            \draw[-,dashed] plot[domain=0:400,samples=41,smooth]({2*sin(\x)},{2*cos(\x)},{0});
            % Circle in x, z plane
            \draw[-,dashed] plot[domain=0:400,samples=41,smooth]({2*cos(0)*sin(\x)},{2*sin(0)*sin(\x)},{2*cos(\x)});
            % Circle at top
            \draw[-,dashed] plot[domain=0:360,samples=41,smooth]({1.414*sin(\x)},{1.414*cos(\x)},{1.414});
        ...
        \begin{scope}
            ...
            \draw[-,dashed] plot[domain=0:400,samples=41,smooth]({2*sin(\x)},{2*cos(\x)});
            %Basis vector lables
    ...
    \caption{Spherical Coordinates. \((x',y',z')\) indicate the directions of the basis vectors in the global cartesian coordinate system, while \(\hat{r},\hat{\phi},\hat{\lambda}\) indicate the basis vectors of the spherical coordinate system in magenta arrows. As seen, these basis vectors are space-dependent, and change direction depending on where you are on Earth.}
    \label{LocCartes}
\end{figure}
...
We identify the basis vectors \(\hat{r}\), \(\hat{\phi}\), \(\hat{\lambda}\) as pointing in the radial (upward away from the centre of the Earth), northward, and eastward directions.\footnote{
    Note that I go against common mathematical convention here by using \(\phi=\frac{\pi}{2}-\theta\) where \(\theta=\) the polar angle. I do this here to follow convention in the GFD literature.
}. We commonly refer to the east-west direction as the \textbf{zonal} direction (the direction of \(\hat{\lambda}\)) and the north-south direction (the direction of \(\hat{\phi}\)) as the \textbf{meridional} direction. Explicitly, they are given as follows (in matrix form for brevity):
\begin{align*}
    \left( \begin{array}{c}
        \hat{r}
        \\
...
Again, the velocity vector is given by: \(\vec{u}=w\,\hat{r}+v\,\hat{\phi}+u\,\hat{\lambda}\).
