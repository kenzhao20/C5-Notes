...
Our final assumption is regarding the compressibility of fluids. If the mass of a fluid element is conserved following the flow (i.e., \(\frac{D\rho}{Dt}=0\) holds) then, by Equation \ref{Mass Conservation}, \(\vec{\nabla}\cdot\vec{u}=0\) also holds. We say that this fluid is \textbf{incompressible}: every bit of fluid that enters some volume must also leave.
...
However, we are justified to use this equation if we assume that density variations \(\delta p = \delta p (x,y,z,t)\) are much smaller than the background density \(\rho_0\). We therefore assume that the following relation holds.
\begin{align}
    \boxed{\vec{\nabla}\cdot \vec{u}=0}\label{Incompressible}
\end{align}

However, I should emphasise that this does \textit{not} imply that \(\frac{D\rho}{Dt} = \frac{D\delta \rho}{Dt} = 0\) (but it also doesn't prohibit it). More discussion is in the Appendix in the section regarding the \hyperref[Appendix Thermodynamic Eqn]{Thermodynamic Equation}.
...
After eliminating the small terms, we arrive at our final set of simplified equations.
