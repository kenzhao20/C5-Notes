...
While we can solve this algebraically (it is not difficult), we will gain more understanding from solving this graphically, by understanding the \(\alpha \vec{u}_h\) as a kind of small `correction' term on the existing geostrophic winds.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\linewidth}
        ...
        \caption{Anti-Cyclone: a \textbf{High} Pressure System. The fluid flows clockwise around an anti-cyclone in the northern hemisphere.}
    \end{subfigure}
    \caption{[UNDER CONSTRUCTION] Cyclones and Anticyclones in the northern hemisphere (\(f>0\)) near the surface accounting for the effects of friction. Isobars are drawn as solid black lines \textcolor{black}{\rule{0.25cm}{0.25cm}}, the pressure gradient force \(\left( -\frac{1}{\rho}\vec{\nabla}_h p \right)\) as \textcolor{mymagenta}{thick dotted magenta arrows \rule{0.25cm}{0.25cm}}, the coriolis force \(\left( -f\vec{k}\times\vec{u}_h \right)\) as \textcolor{myorange}{thick dashed orange arrows \rule{0.25cm}{0.25cm}}, and the velocity \(\left( \vec{u}_h \right)\) as \textcolor{mydarkblue}{thick filled dark blue arrows \rule{0.25cm}{0.25cm}}.}
\end{figure}
