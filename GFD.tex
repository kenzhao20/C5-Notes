...
We can only take the curl of a \textbf{vector field}, and the curl is always a \textbf{vector field}.\endnote{
    I should note that we can talk about this more generally using the wedge product, in which case the curl of a vector field will always be a bivector field. This has the added benefit of being generalisable to two dimensional vector fields \(\vec{u}=(u,v)^T\) where the curl equals a (pseudo-)scalar $\zeta$. However, this has the downside of confusing the reader (and the author!) if they're not familiar with bivectors and wedge products.

    You might ask how useful it is to talk about two-dimensional fluid flow, in which case I would say very, given certain properties of fluid flow on a rotating gravity-causing planet with a shallow atmosphere like ours!
}
...
Note crucially that this only measures the \textbf{local} rotation, not the global rotation. The example often given to explain this distinction is a paddleboard: consider a very small paddleboard suspended in a fluid with a velocity field \(\vec{u}(\vec{r})\). The paddleboard is pushed around by the flow. The curl of \(\vec{u}\) encodes whether the paddleboard \textit{itself} will turn, not whether it will follow a circular path.

\subsection{The Divergence}

We can only take the divergence of a \textbf{vector field}, and the divergence is always a \textbf{scalar field}.

Broadly, the divergence measures the local `spreading out' properties of a \textbf{vector field}; the higher the divergence, the more the vector field is `spreading out' at that point. 

I find the divergence theorem explains divergence the easiest:
\begin{align}
    \label{Div Thm}
    \iiint_v \vec{\nabla}\cdot \vec{u} \,dV = \iint_s \vec{u}\cdot d\vec{S}
\end{align}
...
\end{align}
The Material Derivative of a quantity is the time-derivative of a quantity \textit{following a fluid element}.\footnote{
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node[] at (0,5.3) {North Pole};
        \node[] at (5.8,0.3) {Equator};
        \node[] at (5,-0.3) {Top of Atmosphere};
        \node[] at (-0.8,3) {Ground};
        \draw (0,0) -- (5,0) arc[start angle=0, end angle=90, radius=5cm] -- cycle;
        \draw (0,0) -- (3,0) arc[start angle=0, end angle=90, radius=3cm] -- cycle;
        \draw[ultra thick,mymagenta,->] (4,0.3) .. controls +(0.7,0) and +(0.5,-0.5) .. (3.3,3.3);
        \draw[ultra thick,mymagenta,->] (3.3,3.3) .. controls +(-0.5,0.5) and +(0,0.7) .. (0.3,4);
        \draw[ultra thick,mymagenta,->] (0.3,4) .. controls +(0,-0.7) and +(-0.5,0.5) .. (2.5,2.5);
        \draw[ultra thick,mymagenta,->] (2.5,2.5) .. controls +(0.5,-0.5) and +(-0.7,0) .. (4,0.3);
        % \draw[thick,mymagenta,->] (4,0.3) -- (3.3,3.3) -- (0.3,4) -- (2.5,2.5) -- cycle;
        % \draw[thick,->] (p0) .. controls +(0,1) and +(1,0) .. (LFC);
        %\draw (3,0) -- (0,0) -- (0,3);
    \end{tikzpicture}
    \caption{Hadley's Convection Cell Model: heat is transported poleward by air parcels which are heated near the equator, rise, then flow poleward, before sinking and flowing southward to complete the loop. As the air parcels flow north/southward, they acquire a zonal velocity due to the rotation of the Earth.}
\end{figure}
...
\section{Transforming Coordinate Systems}\label{s Transforming EoM}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.45\linewidth}
        \centering
        \begin{tikzpicture}
            \draw[->] (0,0) -- (3.5,0) node[anchor=west] {\(x'\)};
            \draw[->] (0,0) -- (0,3.5) node[anchor=south] {\(y'\)};
            \draw[thick,->] (0,0) -- (3,2) node[anchor= north] {\(\vec{A}(t)\)};
            \draw[thick,->] (0,0) -- (2,3) node[anchor=south] {\(\vec{A}(t+\delta t)\)};
            \draw[ultra thick,->] (3,2) -- (2,3) {};
            \node[] at (3.5,2.75) {\(\delta t \left( \frac{d \vec{A}}{d t} \right)_R\)};
        \end{tikzpicture}
    ...
    \begin{subfigure}{0.45\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \draw[->,dashed] (0,0) -- (3.289,1.197) node[anchor=west] {\(x'\)};
            \draw[->,dashed] (0,0) -- (-1.197,3.289) node[anchor=east] {\(y'\)};
            \draw[ultra thick,->] (3,2) -- (0.853,3.503);
            \node[] at (1.5,2) {\(\delta t \left( \frac{d \vec{A}}{d t} \right)_I\)};
        \end{tikzpicture}
...
\subsection{Space-Dependent Coordinate Systems Pt. 1: Spherical Coordinate System}\label{s Spherical Coordinates}
...
We will introduce three coordinate systems, all of which rotate with the Earth.\footnote{We do not have to worry again here regarding this, as partial derivatives are linear, so we can just add the effects together.} This will be a bit convoluted, but we will only introduce the first two coordinate systems in this section (\hyperref[s Spherical Coordinates]{Space-Dependent Coordinate Systems Pt. 1}). We will then introduce the third coordinate system in \hyperref[s Local Cartesian Coordinates]{Space-Dependent Coordinate Systems Pt. 2}

First is the \textbf{global cartesian coordinate system}, with basis vectors \(\hat{x}',\hat{y}',\hat{z}'\) in the directions shown in Figure \ref{LocCartes}. The origin is located at the centre of the Earth, and the basis vectors rotate with the Earth (i.e., they rotate with angular velocity \(\vec{\Omega}=|\Omega|\hat{z}'\)). The basis vectors do vary with time (as they're rotating with the Earth), but \textit{do not} vary with space. This is your classic 3D cartesian coordinate system, and the equations of motion in this frame of reference are straightforwardly the \(x\),\(y\), and \(z\) components of Equation \ref{Navier Stokes Rotating}.

Second, we introduce the \textbf{spherical coordinate system}. Again, the origin is located at the centre of the Earth, and the basis vectors rotate with the Earth (i.e., they rotate with angular velocity \(\vec{\Omega}\)). However, this time, the basis vectors also \textit{vary with space}. The basis vectors at each point in space align themselves with the surface of the Earth as shown by the magenta arrows in Figure \ref{LocCartes}. Notice how the orientation of the magenta arrows change depending on the point. This is your classic 3D curvilinear spherical coordinate system.

\begin{figure}[H]
    \centering
    \scalebox{1.4}{
    \begin{tikzpicture},
        \begin{scope}[3d view={110}{25.26}]
            % Lambda Arc
            % \tdplotsetmaincoords{0}{0}
            \tdplotsetmaincoords{65.26}{110}
            \tdplotdrawarc{(0,0,0)}{1}{0}{330}{anchor=north}{$\lambda$}
            % \draw (0,0) -- (3,0) arc[start angle=0, end angle=90, radius=3cm] -- cycle;
            % \drawfill[->,fill=myyellow] (0,0) -- (0.7,0.7) arc[start angle=0, end angle=225, radius=1cm] -- (0,0);
        \end{scope}
        \begin{scope}[3d view={110}{25.26}]
            % Global Cartesian Coordinate System
            \draw[->,opacity=0.7] (0,0,0) -- (2,0,0) node[anchor=north] {\(x'\)};
            \draw[->,opacity=0.7] (0,0,0) -- (0,2,0) node[anchor=north] {\(y'\)};
            \draw[->,opacity=0.7] (0,0,0) -- (0,0,2) node[anchor=north west] {\(z'\)};
            %Global Cartesian Basis Vectors
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0.5,0,0) node[anchor=north] {\(\hat{x}'\)};
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0,0.5,0) node[anchor=north] {\(\hat{y}'\)};
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0,0,0.5) node[anchor=south west] {\(\hat{z}'\)};
        \end{scope}
        \begin{scope}[3d view={140}{25.26}]
            % Circle in x-y plane
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*sin(\x)},{2*cos(\x)},{0});
            % Circle in x, z plane
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*cos(0)*sin(\x)},{2*sin(0)*sin(\x)},{2*cos(\x)});
            % Circle at top
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({1.414*sin(\x)},{1.414*cos(\x)},{1.414});
            %Spherical Basis Vectors
            \coordinate (A) at (1.414,0,1.414);
            \draw[-] (0,0,0) -- (A);
            \draw[->,thick,mymagenta] (A) -- ++(-0.3536,0,0.3536);
            \draw[->,thick,mymagenta] (A) -- ++ (0,0.5,0);
            \draw[->,thick,mymagenta] (A) -- ++ (0.3536,0,0.3536);
            %\filldraw[fill=myorange,opacity=0.5] (1,0,0) .. controls +(0,0,0.8) and +(0.5,0.2,0) .. (0.7071,0,0.7071) -- (0,0,0);
            %Phi Arc
            % \filldraw[fill=myorange,opacity=0.5] (0,0,0) -- (0.7071,0,0.7071) arc[start angle=45, end angle=0, radius=1cm] -- cycle;
            \filldraw[fill=myorange,opacity=0.5] (1,0,0) .. controls +(0,0,0.5) and +(0.1,0,-0.1) .. (0.7071,0,0.7071) -- (0,0,0) -- cycle;
            % %Lambda Arc
            % \filldraw[fill=myyellow,opacity=0.5] (1,0,0) .. controls +(0,0.5,0) and +(0.1,0.1,0) .. (0.866,0.5,0) -- (0,0,0) -- cycle;
        \end{scope}
        \begin{scope}
            %Big circle on outside
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*sin(\x)},{2*cos(\x)});
            %Basis vector labels
            \node[mymagenta] at (-1.05,1.45) {\(\hat{\phi}\)};
            \node[mymagenta] at (-0.65,1.1) {\(\hat{\lambda}\)};
            \node[mymagenta] at (-1.5,0.95) {\(\hat{r}\)};
            %Coordinate labels
            \node at (-0.45,0.08) {\(\phi\)};
            % \node at (-0.4,-0.22) {\(\lambda\)};
            \node at (-0.3,0.5) {\(r\)};
        \end{scope}
    \end{tikzpicture}}
    \hspace{2cm}
    ...
    \begin{tikzpicture},
        \begin{scope}[3d view={110}{25.26}]
            ...
            % \tdplotsetmaincoords{0}{0}
            \tdplotsetmaincoords{65.26}{110}
            \tdplotdrawarc{(0,0,0)}{1}{0}{110}{anchor=north}{}
            % \draw (0,0) -- (3,0) arc[start angle=0, end angle=90, radius=3cm] -- cycle;
            % \drawfill[->,fill=myyellow] (0,0) -- (0.7,0.7) arc[start angle=0, end angle=225, radius=1cm] -- (0,0);
        \end{scope}
        \begin{scope}[3d view={110}{25.26}]
            % Global Cartesian Coordinate System
            \draw[->,opacity=0.7] (0,0,0) -- (2,0,0) node[anchor=south] {\(x'\)};
            \draw[->,opacity=0.7] (0,0,0) -- (0,2,0) node[anchor=south] {\(y'\)};
            \draw[->,opacity=0.7] (0,0,0) -- (0,0,2) node[anchor= west] {\(z'\)};
            %Global Cartesian Basis Vectors
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0.5,0,0) node[anchor=south] {\(\hat{x}'\)};
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0,0.5,0) node[anchor=south] {\(\hat{y}'\)};
            \draw[->,opacity=0.7,mydarkblue, thick] (0,0,0) -- (0,0,0.5) node[anchor=south west] {\(\hat{z}'\)};
        \end{scope}
        \begin{scope}[3d view={0}{25.26}]
            % Circle in x-y plane
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*sin(\x)},{2*cos(\x)},{0});
            % Circle in x, z plane
            \draw[-,dashed] plot[domain=0:400,samples=41,smooth]({2*cos(0)*sin(\x)},{2*sin(0)*sin(\x)},{2*cos(\x)});
            % Circle at bottom
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({1.414*sin(\x)},{1.414*cos(\x)},{-1.414});
            %Spherical Basis Vectors
            \coordinate (A) at (1.414,0,-1.414);
        ...
        \begin{scope}
            ...
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*sin(\x)},{2*cos(\x)});
            %Basis vector lables
            \node[mymagenta] at (1.9,-1) {\(\hat{\phi}\)};
            \node[mymagenta] at (1.4,-0.85) {\(\hat{\lambda}\)};
            \node[mymagenta] at (2,-1.5) {\(\hat{r}\)};
            %Coordinate labels
            \node at (0.75,-0.35) {\(\phi\)};
            \node at (0.2,-0.6) {\(\lambda\)};
            \node at (0.9,-1) {\(r\)};
        \end{scope}
    \end{tikzpicture}
    % \begin{tikzpicture},
    ...
    \caption{Spherical Coordinates. \((x',y',z')\) indicate the directions of the basis vectors in the global cartesian coordinate system, while \(\hat{r},\hat{\phi},\hat{\lambda}\) indicate the basis vectors of the spherical coordinate system in magenta arrows. Compare the image on the left to the image on the right: as you can see, the spherical basis vectors (the magenta arrows) are space-dependent, and point in different directions depending on where you are on Earth (i.e., they depend explicitly on the position vector \(\vec{r}\)).}
    \label{LocCartes}
\end{figure}

We identify the basis vectors \(\hat{r}\), \(\hat{\phi}\), \(\hat{\lambda}\) as pointing in the radial (upward away from the centre of the Earth), northward, and eastward directions.\footnote{
    Note that I go against common mathematical convention here by using \(\phi=\frac{\pi}{2}-\theta\) where \(\theta=\) the polar angle. I do this here to follow convention in the GFD literature. If you refer to, for example, \href{https://en.wikipedia.org/wiki/Vector_fields_in_cylindrical_and_spherical_coordinates}{this}  wikipedia page, you will notice some of the matrices look different. This is most likely due to the difference in polar angle definition, and to convert simply recall that \(\sin\theta = \cos\phi\) and \(\cos\theta = \sin\phi\).
}. We commonly refer to the east-west direction as the \textbf{zonal} direction (the direction of \(\hat{\lambda}\)) and the north-south direction (the direction of \(\hat{\phi}\)) as the \textbf{meridional} direction.

In this coordinate system, we define the position of a fluid parcel on the Earth by the coordinates \((r,\phi,\lambda)\), indicating the radial distance (from the centre of the Earth), the latitude \(\phi\in[-\frac{\pi}{2},\frac{\pi}{2}]\) (\(\phi=0\) indicates the equator, \(\phi=\pm\frac{\pi}{2}\) is the north/south pole), and the longitude \(\lambda\in[0,2\pi]\). The position vector is therefore given by: \(\vec{r}=r\,\hat{r}+\phi\,\hat{\phi}+\lambda\,\hat{\lambda}\). The coordinates reprsenting the velocity of the fluid parcel is given by \((u,v,w)\) as the velocity in the east, north, and radial directions. They are given explicitly as follows:

\vspace{-4mm}
\begin{minipage}{.31\linewidth}
\begin{align*}
    u = r\,\cos\phi\,\dot{\lambda}
\end{align*}
\end{minipage}
\begin{minipage}{.31\linewidth}
\begin{align*}
...
Again, the velocity vector is given by: \(\vec{u}=w\,\hat{r}+v\,\hat{\phi}+u\,\hat{\lambda}\). The explicit form of the basis vectors are given in this\footnote{
    The explicit form of the basis vectors \(\hat{r},\hat{\phi},\hat{\lambda}\) are given here. Note that I put this here for completeness, but all the intuition you need is in Figure \ref{LocCartes}. 
    \begin{align*}
        \left( \begin{array}{c}
            \hat{r}
            \\
        ...
        \end{array} \right)
    \end{align*}
    with the inverse transformation being (the inverse is simply the transpose, since it is an orthogonal matrix):
    \begin{align*}
        \left( \begin{array}{c}
            \hat{x}
            \\
...
% In this coordinate system, we define the position and velocity coordiates \((x,y,z)\) and \((u,v,w)\) as indicating distance and velocity in the east, north, and upwards directions, respectively. \(r=\) the radial coordinate, identifying the distance from the centre of the Earth, \(\) 
...
% Explicitly, \(\vec{r}=(x,y,z)=x\,\hat{x}+y\,\hat{y}+z\,\hat{z}\) are given by (where \(\lambda=\) the longitude):
...
% \vspace{1mm}\noindent
Our goal now is to derive which extra terms appear in the equations of motion if we write it in terms of this spherical coordinate system.

The first step is to consider the material derivative \(\frac{D\vec{u}}{Dt}\), taking into account that it is not necessarily the case that \(\frac{D\hat{r}}{Dt}=\frac{D\hat{\phi}}{Dt}=\frac{D\hat{\lambda}}{Dt}=0\) and again use product rule:
\begin{align*}
    \frac{D\vec{u}}{Dt} = 
    \frac{Du}{Dt}\hat{\lambda} +
    \frac{Dv}{Dt}\hat{\phi} +
    \frac{Dw}{Dt}\hat{r} +
    u\frac{D\hat{\lambda}}{Dt} +
    v\frac{D\hat{\phi}}{Dt} +
    w\frac{D\hat{r}}{Dt}
\end{align*}
We now require knowledge about these basis vectors \(\hat{\lambda}\), \(\hat{\phi}\), \(\hat{r}\) vary with the movement of the fluid parcel, i.e., the precise value of \(\frac{D\hat{\lambda}}{Dt}\). Writing this out explicitly\footnote{For an alternative treatment, refer to \href{https://farside.ph.utexas.edu/teaching/336L/Fluidhtml/node258.html}{this}.}:
\begin{align*}
    \frac{D\hat{\lambda}}{Dt}
    =
    \frac{\partial \hat{\lambda}}{\partial t}
    +
    u\frac{1}{r\,\cos\phi}\frac{\partial \hat{\lambda}}{\partial \lambda}
    +
    v\frac{1}{r}\frac{\partial \hat{\lambda}}{\partial \phi}
    +
    w\frac{\partial \hat{\lambda}}{\partial r}
\end{align*}
The time derivatives are zero (\(\frac{\partial \hat{\lambda}}{\partial t}=\frac{\partial \hat{\phi}}{\partial t}=\frac{\partial \hat{r}}{\partial t}=0\)) since the basis vectors do not change in time (relative to the rotating coordinates \(x',y',z'\)). The spatial derivatives (exercise, but probably not worth it tbh) can be found geometrically by writing the spherical basis vectors in terms of the cartesian basis vectors (using the transformation in the footnote below), differentiating normally, then converting back to spherical coordinates (using the inverse transformation in the footnote below). Therefore: 
[UNDER CONSTRUCTION]

\vspace{-3mm}
\begin{minipage}{.3\linewidth}
\begin{align*}
    &\frac{\partial \hat{\lambda}}{\partial \lambda}=-\sin(\phi) \, \hat{\phi}-\cos(\phi)\hat{r}
    \\
    &\frac{\partial \hat{\lambda}}{\partial \phi}=0
    \\
    &\frac{\partial \hat{\lambda}}{\partial r}=0
    \\
    \therefore\,
    &
...
\begin{minipage}{.3\linewidth}
\begin{align*}
    &\frac{\partial \hat{\phi}}{\partial \lambda}=-\sin(\phi)\,\hat{\lambda}
    \\
    &\frac{\partial \hat{\phi}}{\partial \phi}=-\hat{r}
    \\
...
\begin{minipage}{.3\linewidth}
\begin{align*}
    &\frac{\partial \hat{r}}{\partial \lambda}=\cos\phi\hat{\lambda}
    \\
...
\begin{align*}
    \vec{\Omega} = \Omega \,(\hat{\phi}\,\cos\phi+\hat{r}\,\sin\phi)
\end{align*}

Plugging into Equation \ref{Navier Stokes Rotating}, churning through the algebra, and equating components, we get:
\begin{align*}
    \frac{Du}{Dt}-\left( 2\Omega+\frac{u}{r\cos\phi} \right)\left( v\sin\phi-w\cos\phi \right)+\frac{1}{\rho r \cos\phi}\frac{\partial p}{\partial \lambda}
    =F_\lambda
    \\
    \frac{Dv}{Dt}+\frac{wv}{r}+\left( 2\Omega+\frac{u}{r\cos\phi} \right)u\sin\phi+\frac{1}{\rho r}\frac{\partial p}{\partial \phi}=F_\phi
    \\
...
Note that, unlike before, Equations \ref{Mass Conservation} - \ref{Energy Equation} \textit{do} look substantially different, but only for the reason that the pressure gradient term looks different. The material derivative terms (e.g., the \(\frac{D\rho}{Dt}\)) don't look different because scalars like \(\rho\) are unchanged under our coordinate transformation.

\section{Simplifying the Equations of Motion}
...
\hline
\hline
Horizontal Scale & \(L\) & \(\frac{1}{r \cos\phi}\frac{\partial}{\partial \lambda}\sim\frac{1}{L}\), \(\frac{1}{r}\frac{\partial}{\partial \phi}\sim\frac{1}{L}\)& \qty{e6}{\metre}\\
\hline
...
\subsection{Space-Dependent Coordinate Systems Pt. 2: Local Cartesian Coordinate Systems}\label{s Local Cartesian Coordinates}

We now, finally, post-scale analysis, introduce the third coordinate system: the \textbf{Local Cartesian Coordinate System}. This coordinate system has the origin located not at the centre of the Earth, but about the location of interest on the sphere. The basis vectors, again, rotate with the Earth. However, the basis vectors are constant with space. The basis vectors for the \textbf{Local Cartesian Coordinate System} then, \textit{about a certain point}, are shown by the magenta arrows in Figure \ref{LocCartes}.

Again, we identify the basis vectors \(\hat{x}\), \(\hat{y}\), \(\hat{z}\) as pointing in the eastward, northward, and upward (away from the centre of the Earth) directions \textit{at the origin} of the coordinate system. The further we stray from the origin, the more those basis vectors point in different directions! In this coordinate system, we define the position and velocity coordiates \((x,y,z)\) and \((u,v,w)\) as indicating distance and velocity in the east, north, and upwards directions, respectively. We also refer to the east-west direction as the \textbf{zonal} direction and the north-south direction as the \textbf{meridional} direction.

I think this is how we define it in the lecture slides and in many textbooks. 

I like to think of it another way, but, again, this is written by me so 

Another way to think about this coordinate system is as mathematically (but not conceptually) approximating the derivatives: due to the shallow aspect ratio, we can approximate, for example, \(\frac{1}{r}\frac{\partial}{\partial\phi}\sim\frac{\partial}{\partial y}\).
...
\begin{figure}[H]
    ...
    \begin{tikzpicture},
        ...
        \begin{scope}
            ...
            \draw[-,dashed] plot[domain=0:360,samples=37,smooth]({2*sin(\x)},{2*cos(\x)});
            %Basis vector labels
            \node[mymagenta] at (-1.05,1.45) {\(\hat{y}\)};
            \node[mymagenta] at (-0.65,1.1) {\(\hat{x}\)};
            \node[mymagenta] at (-1.5,0.95) {\(\hat{z}\)};
            %Coordinate labels
            \node at (-0.45,0.08) {\(\phi\)};
            % \node at (-0.4,-0.22) {\(\lambda\)};
            \node at (-0.3,0.5) {\(r\)};
        \end{scope}
    \end{tikzpicture}}
    \caption{Local Cartesian Coordinates. \((x',y',z')\) indicate the directions of the basis vectors in the \textbf{global} cartesian coordinate system, while \(\hat{x},\hat{y},\hat{z}\) indicate the basis vectors of the \textbf{local} cartesian coordinate system in magenta arrows.}
    \label{LocCartes2}
\end{figure}

We finally derive the simplified `primitive' equations of motion.
...
\begin{align*}
    t=[t]\,\hat{t}\hspace{2mm}\text{;}\hspace{2mm}
    \vec{r}=L\,\vec{\hat{r}}\hspace{2mm}\text{;}\hspace{2mm}
    \vec{u}_h=[u_h]\,\vec{\hat{u}}_h\hspace{2mm}\text{;}\hspace{2mm}
    p=[p]\,\hat{p}
\end{align*}
...
} We now substitute these variables into \ref{Horizontal Approximate}\footnote{
    The astute reader will notice the sleight of hand of replacing the \(\vec{u}\cdot\vec{\nabla}\) term with a \(\vec{u}_h\cdot\vec{\nabla}\). In other words, I've neglected vertical advection of horizontal momentum. I've done this to avoid getting too bogged down in the details.

    If you do want to get bogged down in the details, we would have to introduce another vertical velocity and length scale \(W\) and \(H\) for vertical advection. We then must assume that \(\frac{W}{H}\sim\frac{U}{L}\), which will give us the same Rossby number. We can justify this latter assumption by, for example, assuming an advective timescale, or justifying this by scale analysis of Equation \ref{Mass Conservation}.
}:
\begin{align*}
...
\begin{align}
    \label{Mass Conservation Pressure}
    \BOX{
...
It's useful to just go over some examples to gain some intuition regarding pressure coordinates. What is this `geopotential height' object (how could the height \(z\) enter dynamically into the equations of motion?) and why does mass conservation take such a simple and \textit{different} form in pressure coordinates?

Let's consider the first issue. From Equation \ref{Geostrophic Pressure}, we can see that the horizontal gradients of pressure (at constant height) are equal to horizontal gradients of geopotential (at constant pressure), up to a factor of \(rho\). Let's try to understand this by considering an example.

Consider a system where a given pressure surface \(p\) (say, \qty{500}{\hecto\pascal}) is at some height \(z+\Delta z\) (say, \qty{6}{\kilo\meter}) at some position \((x,y)\), and its surroundings are at some lower height \(z\) (say, \qty{5}{\kilo\meter}). The system is approximately hydrostatic, so everywhere pressure monotonically increases with height. 

[UNDER CONSTRUCTION]

You can see in the next chapter that the intuition for pressure coordinates will be similar to how we think about the fluid height \(\eta\). This is not a coincidence -- the intuition is the same because both systems require hydrostasy: pressure is only a monotonic coordinate if the system is approximately hydrostatic, and the shallow water system assumes explicitly that the system is hydrostatic.

We now turn to the second issue. Mass conservation takes a very different form in pressure coordinates (compare Equation \ref{Mass Conservation Pressure} to Equation \ref{Mass Conservation}). In the pressure coordinates version, there are no time-derivatives, while in the height corodinates version, there are time-derivatives! 
...
We first take the \(z\)-component of the curl of Equation \ref{SW mom} (and recalling that \(f\neq const\)):
\begin{align*}
...
We can interpret material conservation of potential vorticity by considering an fluid element: a column of fluid.

This column of fluid lies between the bottom (\(z=b\)) and top (\(z=\eta\)) of the layer, and is infintesimally thin. We impose that the column is infinitesimally thin to ensure that \(u\), \(v\), and \(h\) do not vary appreciably over the entire column. However, we are free to consider the entire column of fluid because of our assumption (in the derivation) that \(u\) and \(v\) do not vary with \(z\).

As this column of fluid is advected around, it must conserve its angular momentum in the \(z\)-direction (since all forces are conservative and the fluid is inviscid). Suppose the column is initially rotating with some (total) angular velocity. If the column is stretched (i.e., if \(h\) increases), it become thinner (because the total volume of the column remains constant), and so it must therefore rotate faster to conserve angular momentum.
