...
Let us then consider a extremely simple model of this: \textbf{Hadley's Model}\footnote{
    I should note that this is not \textit{exactly} Hadley's model. Hadley's model originally assumed that an air parcel's \textbf{linear momentum} is conserved, but we will assume that an air parcel's \textbf{angular momentum} is conserved. See \cite{Hadley_Model} (and remember that the letters that look like f's are s's).
...
I should note that I actually have reservations regarding how useful or even consistent this model is. However, I include this becuase it is on the course.
...
The general moral of the story is: there are conservation laws that constrain motion more on rotating planets than non-rotating planets. 

However, as already mentioned, I'm not actually sure that this is a very good example. Firstly, it seems highly unrealistic to assume that the parcel conserves its angular momentum, given the fact that it must be pushed (presumably) by some force to travel poleward. Secondly, [UNDER CONSTRUCTION]
...
We identify the basis vectors 


We identify the basis vectors \(\hat{x}\), \(\hat{y}\), \(\hat{z}\) as pointing in the eastward, northward, and upward (away from the centre of the Earth) directions. In this coordinate
...
As we can see then, the effects of friction cause the surface of cyclones to be horizontally convergent, while anticyclones are horizontally divergent. Since the fluid is approximately incompressible, there must be vertical ascent of air in cyclones and horizontal descent of air in anticyclones.

In both cases, the dynamics forces vertcial ascent or descent of air parcels. 

In the former case, this can cause condensation, cloud formation, and therefore precipitation. It can also provide the energy needed to overcome \hyperref[Convection Box]{CIN} so that the parcels can access the \hyperref[Convection Box]{CAPE}. This is why cyclones are often associated with more unsettled weather.

This is also an mechanism whereby cyclones and anti-cyclones are dissipated, but we
...
\section{Incompressibility (Approximate Mass Conservation)}

I should strongly emphasise that this approximation can only reasonably apply to either the ocean, or to thin layers of the atmosphere. 

If the mass of a fluid element is conserved following the flow (i.e., \(\frac{D\rho}{Dt}=0\) holds) then, by Equation \ref{Mass Conservation}, \(\vec{\nabla}\cdot\vec{u}=0\) also holds. We say that this fluid is \textbf{incompressible}: every bit of fluid that enters some volume must also leave.
...
More explicitly, in the former case, we know that \(\frac{D\rho}{Dt}\neq 0\) for a rising or falling air parcel, because it will follow the \hyperref[Dry Adiabat Box]{Dry Adiabat Box} and change its density accordingly!

However, we are justified to make this approximation if we assume that density variations \(\delta p = \delta p (x,y,z,t)\) are much smaller than the background density \(\rho_0\). We therefore assume that the following relation holds.

\begin{fact}{Incompressibility}
    If density variations are small in a fluid
    \begin{align}
        \BOX{\vec{\nabla}\cdot \vec{u}=0}\label{Incompressibility}
    \end{align}
    [UNDER CONSTRUCTION]
\end{fact}

However, I should emphasise that this does \textit{not} imply that \(\frac{D\rho}{Dt} = \frac{D\delta \rho}{Dt} = 0\) (but it also doesn't prohibit it), and that this is actually a pretty bad approximation for the atmosphere. More discussion is in the Appendix in the section regarding the \hyperref[Appendix Thermodynamic Eqn]{Thermodynamic Equation}.
...
Physically, hopefully, this makes sense. In the shortwave limit, the wave is effectively a gravity wave, and does not feel the effects of rotation at all. 
